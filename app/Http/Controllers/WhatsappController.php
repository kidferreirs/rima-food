...
class WhatsappController extends BaseRestaurantController
{
    ...
    public function show(string $slug, ConversaWhatsapp $conversa)
    {
        if ($conversa->restaurante_id !== $this->restaurante()->id) {
            abort(403);
        }

        return view('whatsapp.show', compact('conversa'));
    }

    public function simular( Request $request, string $slug, ConversaWhatsapp $conversa, RimaEngine $rima )
    {
        if ($conversa->restaurante_id !== $this->restaurante()->id) {
            abort(403);
        }
