<x-rimafood.layout>
    ...
    <div class="p-8">
        ...
        <div class="flex justify-between items-center mb-8">
            ...
            <a href="{{ route('restaurante.produtos.create', $restauranteAtual->slug) }}" class="bg-green-500 text-white px-5 py-3 rounded-lg">
                + Novo Produto
            </a>
        </div>
                    ...
                    @forelse($produtos as $produto)
                        <tr class="border-b">
                            ...
                            <td class="p-4 flex gap-2">
                                <a href="{{ route('restaurante.produtos.edit', [$restauranteAtual->slug, $produto]) }}"
                                    class="bg-yellow-100 text-yellow-700 px-3 py-2 rounded-lg">
                                    ✏️
                                </a>

                                <form action="{{ route('restaurante.produtos.status', [$restauranteAtual->slug, $produto]) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
