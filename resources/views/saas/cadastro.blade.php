    ...
    <main class="min-h-screen px-4 py-6 sm:px-6 sm:py-8">
        <div class="mx-auto grid w-full max-w-7xl grid-cols-1 gap-6 lg:grid-cols-[1.08fr_.92fr]">
            <section class="rounded-3xl bg-white p-5 shadow-2xl sm:p-8">
                ...
                <p class="mt-3 text-slate-500">Responda algumas perguntas e o Rima Food cria seu ambiente
                    automaticamente.</p>
                ...
                <form id="onboardingForm" action="{{ route('saas.cadastro.store') }}" method="POST" class="mt-7">
                    ...
                    <section class="step-panel" data-step="1">
                        ...
                        <div class="mt-6 space-y-5">
                            <div>
                                ...
                                <input id="restauranteNome" name="restaurante_nome"
                                    value="{{ old('restaurante_nome') }}" placeholder="Ex.: Pizzaria do Amir"
                                    class="mt-1 w-full rounded-xl border border-slate-300 p-3.5 focus:border-orange-500 focus:ring-orange-100"
                                    required>
                            </div>
                            <div>
                                ...
                                <div class="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
                                    @foreach($segmentos as $slug => $segmento)
                                        <label
                                            class="segmento-card cursor-pointer rounded-2xl border-2 border-slate-200 p-3 transition hover:border-orange-300"
                                            data-nome="{{ $segmento['nome'] }}" data-banner="{{ $segmento['banner'] }}"
                                            data-cor="{{ $segmento['cor_primaria'] }}"
                                            data-texto="{{ $segmento['texto'] }}">
                                            <input type="radio" name="segmento" value="{{ $slug }}" class="sr-only"
                                                @checked(old('segmento') === $slug) required>
                        ...
                        <div class="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3">
                            @foreach($plans as $plan)
                                ...
                                <label
                                    ...
                                    <p class="mt-4 text-2xl font-black">R$ {{ number_format($plan->valor, 2, ',', '.') }}
                                    </p>
                    ...
                    <section class="step-panel hidden" data-step="3">
                        ...
                        <div class="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div>
                                <label class="text-sm font-bold">Seu nome</label>
                                <input name="user_nome" value="{{ old('user_nome') }}"
                                    class="mt-1 w-full rounded-xl border border-slate-300 p-3.5" required>
                            </div>
                            <div>
                                <label class="text-sm font-bold">WhatsApp</label>
                    ...
                    <section class="step-panel hidden" data-step="4">
                        ...
                        <div class="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <label class="cardapio-card cursor-pointer rounded-2xl border-2 border-slate-200 p-5"><input
                                    type="radio" name="possui_cardapio" value="sim" class="sr-only"
                                    @checked(old('possui_cardapio', 'sim') === 'sim') required><span
                                    class="text-3xl">📄</span>
                                <p class="mt-3 font-black">Sim, já tenho</p>
                                <p class="mt-1 text-sm text-slate-500">PDF, foto, Excel ou CSV.</p>
                            </label>
                            <label class="cardapio-card cursor-pointer rounded-2xl border-2 border-slate-200 p-5"><input
                                    type="radio" name="possui_cardapio" value="nao" class="sr-only"
                                    @checked(old('possui_cardapio') === 'nao') required><span class="text-3xl">✍️</span>
                                <p class="mt-3 font-black">Vou criar depois</p>
                                <p class="mt-1 text-sm text-slate-500">Entre direto no dashboard.</p>
                            </label>
                        </div>
                        <div class="mt-6 rounded-2xl bg-orange-50 p-5">
                            <p class="font-black text-orange-800">Seu teste começa agora</p>
                            <p class="mt-1 text-sm text-orange-700">15 dias grátis, sem cartão e sem compromisso.</p>
                        </div>
                    </section>

                    <div class="mt-8 flex flex-col-reverse gap-3 sm:flex-row sm:justify-between">
                        <button id="prevButton" type="button"
                            class="hidden rounded-xl border border-slate-300 px-5 py-3 font-bold text-slate-700">Voltar</button>
                        <button id="nextButton" type="button"
                            class="ml-auto rounded-xl bg-orange-500 px-6 py-3 font-black text-white hover:bg-orange-600">Continuar</button>
                        <button id="submitButton" type="submit"
                            class="ml-auto hidden rounded-xl bg-orange-500 px-6 py-3 font-black text-white hover:bg-orange-600">Criar
            ...
            <aside class="lg:sticky lg:top-8 lg:self-start">
                <div class="overflow-hidden rounded-3xl bg-white shadow-2xl">
                    <div id="bannerPreview" class="flex min-h-64 items-end bg-cover bg-center p-6"
                        style="background-image:linear-gradient(180deg,transparent,rgba(15,23,42,.88)),url('/images/onboarding/banners/outro.svg')">
                        <div>
                            <p id="previewSegmento"
                                class="text-sm font-bold uppercase tracking-[.16em] text-orange-300">Seu segmento</p>
                            <h2 id="previewNome" class="mt-2 text-3xl font-black text-white">Seu estabelecimento</h2>
                            <p id="previewTexto" class="mt-2 max-w-md text-sm text-slate-200">Uma identidade inicial
                                será criada automaticamente.</p>
                        </div>
                    </div>
                    <div class="p-6">
                        <div class="flex items-center justify-between gap-4">
                            <div>
                                <p class="text-xs font-bold uppercase tracking-[.16em] text-slate-400">Prévia automática
                                </p>
                                <p class="mt-1 text-lg font-black">Sua marca começa pronta</p>
                            </div>
                            <div id="previewColor" class="h-12 w-12 rounded-2xl bg-orange-500"></div>
                        </div>
                        <ul class="mt-5 space-y-3 text-sm text-slate-600">
                            <li>✓ Banner inicial do segmento</li>
                            <li>✓ Cores aplicadas automaticamente</li>
                            <li>✓ Plano com 15 dias grátis</li>
                            <li>✓ Acesso ao Import Menu</li>
                        </ul>
                    </div>
                </div>
            </aside>
